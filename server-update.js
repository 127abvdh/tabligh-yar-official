...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (!phone || !password) {
      return res.status(400).json({ error: 'شماره تلفن و رمز الزامی است' });
    }
...
// Business Request - Pending Payment
app.post('/api/business-request', async (req, res) => {
  try {
    ...
    if (!amount) {
      return res.status(400).json({ error: 'بسته نامعتبر است' });
    }
    ...
    res.json({ 
      ...
      refId, 
      amount,
      message: 'درخواست ثبت شد. لطفاً مبلغ را به شماره حساب واریز کنید'
    });
  } catch (error) {
    res.status(500).json({ error: 'خطای سرور' });
  }
...
app.post('/api/admin/approve-payment', async (req, res) => {
  try {
    ...
    if (!user || !user.isAdmin) {
      return res.status(403).json({ error: 'فقط ادمین می‌تواند تایید کند' });
    }
    ...
    if (request.paymentStatus === 'approved') {
      return res.status(400).json({ error: 'این درخواست قبلاً تایید شده است' });
    }
    ...
    res.json({ success: true, message: 'درخواست تایید شد، تبلیغ منتشر شد' });
  } catch (error) {
    res.status(500).json({ error: 'خطای سرور' });
  }
});
...
app.get('/api/admin/pending', async (req, res) => {
  try {
    ...
    if (!user || !user.isAdmin) {
      return res.status(403).json({ error: 'فقط ادمین می‌تواند دیدن کند' });
    }
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Vazir', Tahoma; background: #0052cc; color: #333; }
  ...
  <div class="container">
    ...
    <div class="hero">
      ...
      <div class="buttons">
        ...
        <a href="/business-request.html" class="btn btn-secondary">تبلیغ کسب‌وکار</a>
...
<head>
  <meta charset="UTF-8">
  <title>تبلیغ کسب‌وکار</title>
  <style>
    body { font-family: 'Vazir', Tahoma; background: #0052cc; display: flex; align-items: center; justify-content: center; min-height: 100vh; }
...
<body>
  <div class="form">
    <h1>تبلیغ کسب‌وکار</h1>
    <div class="account"><strong>شماره حساب: IR360190000000216518589002</strong></div>
    <form id="form">
      <div class="group">
      ...
      <div class="group">
        ...
        <select id="package" required>
          <option value="sixMonth">۶ ماهه - ۱,۸۰۰,۰۰۰ تومان</option>
          <option value="oneYear">۱ ساله - ۳,۶۰۰,۰۰۰ تومان</option>
        </select>
      </div>
      <div class="group">
  ...
  <script>
    document.getElementById('form').addEventListener('submit', async (e) => {
      ...
      if (data.success) {
        msg.textContent = 'کد: ' + data.refId + ' - لطفاً مبلغ را واریز کنید';
      } else {
        msg.textContent = 'خطا!';
      }
    });
  ...
  <script>
    document.getElementById('form').addEventListener('submit', async (e) => {
      ...
      if (data.success) {
        ...
        document.getElementById('msg').textContent = 'کد معرفی: ' + data.referralCode;
      }
    });
  </script>
</body>
...
<head>
  ...
  <title>داشبورد</title>
  <style>
    body { font-family: 'Vazir', Tahoma; background: #f5f5f5; }
...
<body>
  ...
  <div class="container">
    ...
    <div class="card">
      <h2>درخواست‌ها</h2>
      <table id="table"><thead><tr><th>نام</th><th>بسته</th><th>مبلغ</th></tr></thead><tbody></tbody></table>
    </div>
  ...
  <script>
    async function load() {
      ...
      const data = await res.json();
      document.getElementById('stats').innerHTML = '<div class="stat">' + data.name + '</div><div class="stat">' + data.referralCode + '</div><div class="stat">' + data.totalEarnings.toLocaleString() + ' ت</div>';
...
<body>
  <div class="nav"><h1>داشبورد ادمین</h1></div>
  <div class="container">
    <div class="card">
      <h2>درخواست‌های منتظر پرداخت</h2>
      <table id="table"><thead><tr><th>نام</th><th>شماره</th><th>آدرس</th><th>مبلغ</th><th>عملیات</th></tr></thead><tbody></tbody></table>
    </div>
  ...
  <script>
    async function load() {
      ...
      const data = await res.json();
      data.forEach(r => {
        document.querySelector('#table tbody').innerHTML += '<tr><td>' + r.businessName + '</td><td>' + r.businessPhone + '</td><td>' + r.businessAddress + '</td><td>' + r.amount.toLocaleString() + '</td><td><button class="btn-approve" onclick="approve(\'' + r.refId + '\')">تایید</button></td></tr>';
      });
    }
...
<body>
  ...
  <script>
    async function load() {
      ...
      const data = await res.json();
      if (data.length === 0) {
        document.getElementById('grid').innerHTML = '<p>تبلیغی تایید‌شده‌ای وجود ندارد</p>';
        return;
      }
