...
app.post('/api/business-request', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      message: 'Approved! Transfer to account'
    });
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
...
<body>
  <div class="form">
    ...
    <div class="account">Account: IR360190000000216518589002</div>
    <form id="form">
      <div class="group">
  ...
  <script>
    document.getElementById('form').addEventListener('submit', async (e) => {
      ...
      if (data.success) {
        msg.textContent = 'Reference: ' + data.refId + ' - Transfer to account. Posted in directory!';
      }
    });
  </script>
</body>
...
<body>
  ...
  <script>
    async function load() {
      ...
      const data = await res.json();
      if (data.length === 0) {
        document.getElementById('grid').innerHTML = '<p>No listings yet</p>';
        return;
      }
